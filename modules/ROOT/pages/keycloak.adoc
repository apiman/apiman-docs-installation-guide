...
[MaintainerMessage]
****
If you are interested in support for other SSO providers, please let us know in this https://github.com/apiman/apiman/issues/1704[GitHub Issue^].
****
...
* For simpler configurations, environment variables or Java system properties are available. See <<kc-configuration-options>>.
* For advanced configurations, manual editing of config files may be required. See <<_kc-advanced-configuration>>.
...
NOTE: This is running in dev mode. For production, refer to the {keycloak}[Keycloak Production Guides^].

=== Alternative: Keycloak Quarkus Distro

Alternatively, you can download the bare metal distro.
...
.. You can find the default realm definition in your distribution zip: `apiman/data/apiman-realm-for-keycloak.json`
...
+
For example, if your hostname is `localhost` and your port is `8081`, then your `APIAN_AUTH_URL` would be `http://localhost:8081/`
...
Once the apiman realm has been created or imported, make sure to configure the *Valid Redirect URIs* section of the *apimanui*, *apiman*, *portal*, and *apiman-gateway-api* clients.
...
apiman-developer-portal:: `https://apiman.myorg.com/portal/*`
